...
def read_route_file(filename, g):
	...
	with open (filename) as route:
		reader = csv.DictReader(route,fieldnames=['1','stop_no','distance','express','arr_time','dep_time','first_bus','ID','direction','service_no'])
...
def connect_MRT(graph):
	for line in lines:
		lines[line].sort(key=operator.itemgetter('no'))	
		for current_station, next_station in current_and_next(lines[line]):
			current_name = current_station['Node'] + ' ' + line# + ' 1'
			next_name = next_station['Node'] + ' ' + line# + ' 1'
			edge = graph.add_edge(nodes[current_name]['Index'],nodes[next_name]['Index'])
			lon1 = float(current_station['Long'])
			lat1 = float(current_station['Lat'])
			lon2 = float(next_station['Long'])
			lat2 = float(next_station['Lat'])
			dist = round(haversine(lon1, lat1, lon2, lat2),3)
			
			graph.ep.Distance[edge] = dist
			graph.ep.Method[edge] = 'MRT ' + line
			
			#current_name = next_station['Node'] + ' ' + line + ' 2'
			#next_name = current_station['Node'] + ' ' + line + ' 2'
			edge = graph.add_edge(nodes[next_name]['Index'],nodes[current_name]['Index'])
			graph.ep.Distance[edge] = dist
			graph.ep.Method[edge] = 'MRT ' + line
